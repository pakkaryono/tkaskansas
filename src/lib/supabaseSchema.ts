...
-- 5. FUNGSI RESOLUSI NIS/NIP KE EMAIL (Bypass RLS saat login)
...
DECLARE
    ...
    v_email TEXT;
BEGIN
    IF p_identifier IS NULL OR trim(p_identifier) = '' THEN RETURN NULL; END IF;
    v_clean_id := trim(p_identifier);
    IF v_clean_id LIKE '%@%' THEN RETURN LOWER(v_clean_id); END IF;
...
-- 6. FUNGSI ADMIN: BUAT USER BARU DENGAN SINKRONISASI auth.identities
...
BEGIN
    ...
    -- Sinkronisasi identitas Supabase Auth
    BEGIN
        SELECT EXISTS (
        ...
        IF v_has_provider_id THEN
            ...
            VALUES (v_user_id::text, v_user_id, jsonb_build_object('sub', v_user_id::text, 'email', p_email, 'email_verified', true), 'email', v_user_id::text, NOW(), NOW(), NOW());
        ELSE
            INSERT INTO auth.identities (id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
            VALUES (v_user_id::text, v_user_id, jsonb_build_object('sub', v_user_id::text, 'email', p_email, 'email_verified', true), 'email', NOW(), NOW(), NOW());
        END IF;
    ...
    INSERT INTO public.profiles (id, email, full_name, role, phone_number, nis, nisn, nip, class_name, major_name, status, updated_at)
    VALUES (v_user_id, p_email, p_full_name, p_role, p_phone, p_nis, p_nisn, p_nip, v_class_name, v_major_name, 'active', NOW())
    ON CONFLICT (id) DO UPDATE SET
        email = EXCLUDED.email, full_name = EXCLUDED.full_name, role = EXCLUDED.role,
        phone_number = COALESCE(EXCLUDED.phone_number, public.profiles.phone_number),
        nis = COALESCE(EXCLUDED.nis, public.profiles.nis), nisn = COALESCE(EXCLUDED.nisn, public.profiles.nisn),
        nip = COALESCE(EXCLUDED.nip, public.profiles.nip), class_name = COALESCE(EXCLUDED.class_name, public.profiles.class_name),
        major_name = COALESCE(EXCLUDED.major_name, public.profiles.major_name), status = 'active', updated_at = NOW();

    IF p_role = 'siswa' THEN
        SELECT id INTO v_existing_student_id FROM public.students WHERE LOWER(email) = p_email OR (p_nis IS NOT NULL AND nis = p_nis) LIMIT 1;
        IF v_existing_student_id IS NOT NULL THEN
            UPDATE public.students SET user_id = v_user_id, full_name = p_full_name, email = p_email, phone_number = COALESCE(p_phone, phone_number), nis = COALESCE(p_nis, nis), nisn = COALESCE(p_nisn, nisn), class_id = COALESCE(p_class_id, class_id), major_id = COALESCE(v_major_id, major_id), status = 'active', updated_at = NOW() WHERE id = v_existing_student_id;
        ELSE
            INSERT INTO public.students (id, user_id, nis, nisn, full_name, email, phone_number, class_id, major_id, status, created_at, updated_at)
            VALUES (v_user_id, v_user_id, COALESCE(p_nis, 'S-' || substr(v_user_id::text, 1, 6)), COALESCE(p_nisn, '00' || substr(v_user_id::text, 1, 8)), p_full_name, p_email, p_phone, p_class_id, v_major_id, 'active', NOW(), NOW());
        END IF;
    ELSIF p_role = 'guru' THEN
        SELECT id INTO v_existing_teacher_id FROM public.teachers WHERE LOWER(email) = p_email OR (p_nip IS NOT NULL AND nip = p_nip) LIMIT 1;
        IF v_existing_teacher_id IS NOT NULL THEN
            UPDATE public.teachers SET user_id = v_user_id, full_name = p_full_name, email = p_email, phone_number = COALESCE(p_phone, phone_number), nip = COALESCE(p_nip, nip), status = 'active', updated_at = NOW() WHERE id = v_existing_teacher_id;
        ELSE
            INSERT INTO public.teachers (id, user_id, nip, full_name, email, phone_number, status, created_at, updated_at)
            VALUES (v_user_id, v_user_id, COALESCE(p_nip, 'G-' || substr(v_user_id::text, 1, 8)), p_full_name, p_email, p_phone, 'active', NOW(), NOW());
        END IF;
        ...
        IF p_subject_ids IS NOT NULL AND array_length(p_subject_ids, 1) > 0 THEN
            ...
            SELECT COALESCE(v_existing_teacher_id, v_user_id), unnest(p_subject_ids) ON CONFLICT DO NOTHING;
        END IF;
    END IF;

    RETURN jsonb_build_object('success', true, 'user_id', v_user_id, 'email', p_email, 'role', p_role, 'message', 'Akun berhasil dibuat dan aktif.');
EXCEPTION WHEN OTHERS THEN
...
-- 7. AUTO-REPAIR SINKRONISASI SEMUA IDENTITAS KE auth.identities
DO $$
DECLARE
    u RECORD;
    v_has_provider_id BOOLEAN;
